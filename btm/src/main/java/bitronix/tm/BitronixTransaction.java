...
public class BitronixTransaction implements Transaction, BitronixTransactionMBean {
    ...
    private volatile LocalDateTime timeoutDate;
    ...
    private volatile LocalDateTime startDate;
    ...
    public boolean delistResource(final XAResource xaResource, final int flag) throws IllegalStateException, SystemException {
        ...
        if (!xaResourceHolderStateVisitor.exceptions.isEmpty()) {
            ...
            if (!multiSystemException.isUnilateralRollback()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("unilateral rollback of resource " + resourceHolder, multiSystemException);
                }
            }
        }
    ...
    public void commit() throws RollbackException, HeuristicMixedException, HeuristicRollbackException, SecurityException, SystemException {
        ...
        try {
            ...
            try {
                if (log.isDebugEnabled()) {
                    log.debug("committing, {} enlisted resource(s)", resourceManager.size());
                }
            ...
            // commit phase
            if (log.isDebugEnabled()) {
                log.debug("{} interested resource(s)", interestedResources.size());
            }
    ...
    public void rollback() throws IllegalStateException, SystemException {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("some resource(s) failed delistment", ex);
            }
        }
        ...
        try {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("rolling back, {} enlisted resource(s)", resourceManager.size());
                }
    ...
    public void setActive(long timeout) throws IllegalStateException, SystemException {
        if (status != Status.STATUS_NO_TRANSACTION) {
            throw new IllegalStateException("transaction has already started");
        }

        setStatus(Status.STATUS_ACTIVE);
        this.startDate = Instant.ofEpochMilli(MonotonicClock.currentTimeMillis()).atZone(ZoneId.systemDefault()).toLocalDateTime();
        this.timeoutDate = Instant.ofEpochMilli(MonotonicClock.currentTimeMillis() + (timeout * 1000L))
    ...
    public boolean equals(Object obj) {
        if (obj instanceof BitronixTransaction tx) {
    ...
    private void fireBeforeCompletionEvent() throws BitronixSystemException {
        if (log.isDebugEnabled()) {
            log.debug("before completion, {} synchronization(s) to execute", synchronizationScheduler.size());
        }
    ...
    private void fireAfterCompletionEvent() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("after completion, {} synchronization(s) to execute", synchronizationScheduler.size());
        }
        for (Synchronization synchronization : synchronizationScheduler) {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("executing synchronization {} with status={}", synchronization, Decoder.decodeStatus(status));
                }
    ...
    private boolean isWorking() {
        return switch (status) {
            case Status.STATUS_PREPARING,
                    Status.STATUS_PREPARED,
                    Status.STATUS_COMMITTING,
                    Status.STATUS_ROLLING_BACK -> true;
            default -> false;
        };
    }
